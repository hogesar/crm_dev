...
@section('content')
	...
	foreach($purchase_orders as $purchase_order) {
		...
		$this_products = DB::table('purchase_order_products')->where('purchase_order_id',$purchase_order->id)->where('invoice_created','no')->get();
...
<div class="tab-content">	
	  ...
	  <div id="orderdetails" class="tab-pane fade in active">
		...
		@if ($type == "client")
			<h3 style = "text-align:center;">You are adding an Invoice to the <b>{{ucwords($client->client_name)}}</b> file under deal {{ucwords("ISFI".str_pad($deal->id,6,"0",STR_PAD_LEFT)."P")}}</h3>
		@endif
        ...
        {{ Form::open(array('url' => 'deal/'.$deal->id.'/invoice/store', 'files' => true, 'role'=>'form', 'id'=>'order_form')) }}
        	...
        	<div class = "formSection">
				...
				<div class = "form-group">
					...
					<input type = "date" id = "invoice_date" name = "invoice_date" value = "{{date('Y-m-d')}}" class = "form-control" />
				</div>
