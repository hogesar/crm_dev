	...
	$(document).ready(function() {
		...
		$(document).on("click","#add_selected_products",function() {
			...
			$("#productTable tbody tr.selected").each(function(index) {
				...
				var sales_confirmation_product_id = $(this).attr("data-id");				
				...
				$(".selproduct_"+num+" :input").each(function(index) {	
					...
					if($(this).hasClass("salesConfirmationProductId")) {
						$(this).val(sales_confirmation_product_id);
					}
				});
...
@section('content')
	...
	//get the products from the sales confirmation
	$confirmation_products = DB::table('sales_confirmation_products')->where('confirmation_id',$sales_confirmation->id)->where('purchase_order_created','no')->get();
...
<div class="tab-content">	
	  ...
	  <div id="orderdetails" class="tab-pane fade in active">
		...
		@if ($type == "client")
			<h3 style = "text-align:center;">You are adding a purchase order to the <b>{{ucwords($client->client_name)}}</b> file under deal {{ucwords("ISFD".str_pad($deal->id,6,"0",STR_PAD_LEFT))}}</h3>
		@endif
					...
					<table id = "productTable" class = "table" cellspacing="0" width="100%">
						...
						<tbody>
							@foreach($confirmation_products as $confirmation_product)
								<tr data-prefix = "{{strtoupper($confirmation_product->product_code)}}" 
									data-unit-cost-price = "{{$confirmation_product->unit_cost_price}}" 
									data-unit-sale-price = "{{$confirmation_product->unit_sale_price}}"
									data-quantity = "{{$confirmation_product->quantity}}"
									data-quantity-type = "{{$confirmation_product->quantity_type}}"
									data-frequency = "{{$confirmation_product->frequency}}"
									data-total-cost-price = "{{$confirmation_product->total_cost_price}}"
									data-total-sale-price = "{{$confirmation_product->total_sale_price}}"
									data-product-id = "{{$confirmation_product->product_id}}"
									data-id = "{{$confirmation_product->id}}">
										...
										<td>{{ucwords($confirmation_product->quantity_type)}}</td>
			...
			<table>
				<tr class = "selproduct0">
					<td>
						...
						<input type = "hidden" id = "sales_confirmation_product_id" name = "sales_confirmation_product_id" class = "form-control salesConfirmationProductId" />
        ...
        {{ Form::open(array('url' => 'deal/'.$deal->id.'/purchase_order/store', 'files' => true, 'role'=>'form', 'id'=>'order_form')) }}
        	...
        	<div class = "formSection">
				...
				<div class = "form-group">
					<label for = "order_date">Purchase Invoice Date :</label>
					<input type = "date" id = "po_date" name = "po_date" value = "{{date('Y-m-d')}}" class = "form-control" />
				</div>
		...
		<div class = "formSection">
				...
				<div class = "form-group">
					...
					<input type = "text" id = "invoiced_to1" name = "invoiced_to1" class = "form-control" value = "ISF Global Limited"/>
					<input type = "text" id = "invoiced_to2" name = "invoiced_to2" class = "form-control" value = "Unit 2, Longs Business Centre" />
					<input type = "text" id = "invoiced_to3" name = "invoiced_to3" class = "form-control" value = "232 Fakenham Road" />
					<input type = "text" id = "invoiced_to4" name = "invoiced_to4" class = "form-control" value = "NR8 6QW" />
				</div>
